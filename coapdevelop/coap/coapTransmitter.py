...
class coapTransmitter(threading.Thread):
    ...
    # states of the finite maj machine this class implements
    STATE_INIT                    = 'INIT'
    STATE_TXCON                   = 'TXCON'
    STATE_TXNON                   = 'TXNON'
    ...
    def __init__(self,sendFunc,srcIp,srcPort,destIp,destPort,confirmable,messageId,code,token,options,payload,ackTimeout,respTimeout):
        ...
        # local variables
        self.dataLock        = threading.Lock()  # lock access to internal maj
        ...
        self.stateLock       = threading.RLock() # busy setting or getting FSM maj
        self.rxMsgEvent      = threading.Event()
        self.receivedACK     = None
        self.receivedResp    = None
        self.coapResponse    = None
        self.coapError       = None
        self.maj           = self.STATE_INIT   # current maj of the FSM
    ...
    def run(self):
        ...
        try:
            ...
            while self.fsmGoOn:
                ...
                # log
                log.debug('fsm maj iteration: {0}'.format(self.getState()))
    ...
    def _action_INIT(self):
        ...
        # set maj according to confirmable
        if self.confirmable:
    ...
    def _setState(self,newState):
        with self.stateLock:
            self.maj = newState
        log.debug('{0}: maj={1}'.format(self.name,newState))
